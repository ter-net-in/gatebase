...
pub(crate) async fn list_sessions(
    ...
    require_role(&state, &headers, UserRole::Viewer).await?;
    let sessions = state
        .store
...
pub(crate) async fn get_audit_rollback(
    ...
    Ok(Json(RollbackResponse {
        ...
        before_rows: artifact.before_rows,
...
pub(crate) async fn list_rollbacks(
    ...
    headers: HeaderMap,
    Query(page): Query<Pagination>,
) -> Result<Json<Vec<RollbackResponse>>, ApiError> {
    require_role(&state, &headers, UserRole::Viewer).await?;
    let artifacts = state
        .store
...
pub(crate) async fn list_connections(
    ...
    headers: HeaderMap,
    Query(page): Query<Pagination>,
) -> Result<Json<Vec<ActiveConnectionResponse>>, ApiError> {
    require_role(&state, &headers, UserRole::Viewer).await?;
...
pub(crate) async fn revoke_session(
    ...
    require_role(&state, &headers, UserRole::Operator).await?;
    state
        .store
...
pub(crate) async fn admin_me(
    ...
    let auth = require_role(&state, &headers, UserRole::Viewer).await?;
...
pub(crate) async fn list_users(
    ...
    require_role(&state, &headers, UserRole::Admin).await?;
    let users = state
        .store
...
pub(crate) async fn create_user(
    ...
    require_role(&state, &headers, UserRole::Admin).await?;
    let role = UserRole::from_str(&request.role).map_err(ApiError::bad_request)?;
    let user = state
        .store
...
pub(crate) async fn github_webhook(
    ...
    if !verify_webhook_signature(github.webhook_secret.expose_secret(), &body, signature) {
...
pub(crate) async fn create_session(
    ...
    state
        ...
        .await
        .map_err(ApiError::internal)?;
    let token = state.issuer.issue(&session).map_err(ApiError::internal)?;
    let target = state
        .config
        .targets
        .iter()
        .find(|target| target.name == access_token.target)
        .ok_or_else(|| ApiError::internal(format!("unknown target {}", access_token.target)))?;
    let host = target
        .public_host
        .as_deref()
        .map(str::to_owned)
...
async fn mint_issue_access_token(state: &AppState, repo: &str, issue: i64) -> Result<(), ApiError> {
    let target = state
        .config
        .targets
        .iter()
        .find(|target| target.access.github_repo == repo)
        .ok_or_else(|| ApiError::bad_request(format!("no target configured for repo {repo}")))?;
...
async fn ensure_access_allowed(
    ...
    for signal in &target.access.required_signals {
        ...
        if !evaluation.allowed {
            return Err(ApiError::bad_request(evaluation.reason.unwrap_or_else(
                || format!("required access signal {signal:?} denied"),
            )));
        }
    }
