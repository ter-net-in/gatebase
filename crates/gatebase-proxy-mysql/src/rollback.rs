...
async fn fetch_single_primary_key(upstream: &mut mysql_async::Conn, table: &str) -> Result<String> {
    ...
    let columns: Vec<String> = timeout(QUERY_TIMEOUT, upstream.query(query))
        .await
...
async fn fetch_before_rows(upstream: &mut mysql_async::Conn, query: &str) -> Result<Vec<Value>> {
    let mut result = timeout(QUERY_TIMEOUT, upstream.query_iter(query))
        .await
...
fn row_to_json(columns: &[String], row: &Row) -> Value {
    let mut object = serde_json::Map::new();
    for (index, column) in columns.iter().enumerate() {
        object.insert(
            column.clone(),
            row.as_ref(index).map_or(Value::Null, mysql_value_to_json),
        );
    }
    Value::Object(object)
}
...
fn mysql_value_to_json(value: &MysqlValue) -> Value {
    match value {
        ...
        MysqlValue::Bytes(value) => json!(String::from_utf8_lossy(value).to_string()),
        ...
        MysqlValue::Time(negative, days, hours, minutes, seconds, micros) => {
            ...
            if *micros == 0 {
                json!(format!("{sign}{hours:03}:{minutes:02}:{seconds:02}"))
            } else {
                json!(format!(
                    "{sign}{hours:03}:{minutes:02}:{seconds:02}.{micros:06}"
                ))
            }
        }
    }
